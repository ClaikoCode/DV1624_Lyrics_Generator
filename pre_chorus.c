...
int on_open(const char* fpath, const struct stat* sb, int typeflag)
{
    if (typeflag == FTW_F)
    {
        ...
        *ptr = 0;
        ...
        // risk of pre_chorus not doing well when looking for '_'
        if (strcmp(ptr - 10, "pre_chorus") == 0)
        {
            *ptr = '_';
            ptr = strchr(fpath, '/');
...
char* create_pre_chorus()
{
    char* pre_chorus_text = malloc(BUFFER_LENGTH);
    ...
    char* song_writer = pre_chorus_text;
